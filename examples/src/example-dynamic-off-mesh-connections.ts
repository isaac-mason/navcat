...
let dynamicOffMeshEnabled = true;

const near = (a: Vec3, b: Vec3, r: number) => {
    const dx = a[0] - b[0];
    const dy = a[1] - b[1];
    const dz = a[2] - b[2];
...
// lazily expose off-mesh connections when the path search expands near them
addLazyOffMeshConnections(navMesh, ({ pos }) => {
    if (!dynamicOffMeshEnabled) return null;

    const radius = 2.0; // expose connections whose start is within this radius of current expansion point
    const res: OffMeshConnectionParams[] = [];
    for (let i = 0; i < offMeshConnections.length; i++) {
        const c = offMeshConnections[i]!;
        if (near(pos, c.start, radius)) res.push(c);
    }
...
/* gui */
const gui = new GUI();
const guiState = { dynamicOffMesh: true };
gui
    .add(guiState, 'dynamicOffMesh')
    .name('Dynamic Off-Mesh')
    .onChange(() => {
        dynamicOffMeshEnabled = guiState.dynamicOffMesh;
        // also show/hide helper lines when dynamic off-mesh is toggled
        offMeshConnectionsHelper.object.visible = dynamicOffMeshEnabled;
        updatePath();
    });
// initialize visibility to match dynamic toggle
offMeshConnectionsHelper.object.visible = guiState.dynamicOffMesh;
